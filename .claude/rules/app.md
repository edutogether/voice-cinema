...
- **장르 카드 미리보기를 마우스 호버로만 켜지 말 것** — 이 앱은 부스에서 노트북·태블릿·폰으로
  ...
  기기에서는 `IntersectionObserver`로 화면에 들어온 카드를 **음소거로** 재생한다. 음소거는 타협이
  아니라 필수다 — 소리가 있으면 자동재생이 막혀 정지 화면 그대로가 되고, 부스에서 여섯 개가
  동시에 소리를 낸다. `e2e/tile-autoplay.spec.js`가 이를 고정한다
...
- E2E: `npm run test:e2e` (14개) — 빌드 후 `dist/`를 서빙해 실제 산출물로 검증한다. 최초 1회
