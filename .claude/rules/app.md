...
- **장르 카드 미리보기를 마우스 호버로만 켜지 말 것** — 이 앱은 부스에서 노트북·태블릿·폰으로
  ...
  기기에서는 **화면 세로 가운데에 가장 가까운 카드 한 장**이 PC의 호버 카드 역할을 한다 — 그 한 장만
  **음소거로** 재생되고 강조된다. 음소거는 타협이 아니라 필수다 — 소리가 있으면 자동재생이 막혀
  정지 화면 그대로가 되고, 부스에서 여러 장이 동시에 소리를 낸다.
- **모바일 강조 효과를 별도 CSS로 만들지 말 것** — PC의 `:hover` 규칙에 `.tile.is-active`를 나란히
  붙여 **같은 규칙을 나눠 쓴다**. 새로 만들면 나중에 한쪽만 바뀌어 두 화면이 갈라진다.
  `e2e/tile-autoplay.spec.js`가 두 경로를 다 고정한다(모바일 활성 카드, PC 호버)
...
- E2E: `npm run test:e2e` (16개) — 빌드 후 `dist/`를 서빙해 실제 산출물로 검증한다. 최초 1회
