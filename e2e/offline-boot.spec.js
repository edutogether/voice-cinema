...
test('인터넷이 끊긴 상태에서 탭을 새로 열어도 앱이 정상 부팅된다', async ({ page, context }) => {
  ...
  // 서비스워커가 제어권을 잡은 뒤의 상태에서 확인해야 하므로 한 번 더 명시적으로 연다.
  // (첫 설치 때는 앱이 스스로 새로고침하지 않는다 — 화면이 이미 최신본이라 새로고칠
  //  것이 없는데, 설치가 끝나는 시점에 새로고치면 녹음·업로드 중인 작업이 날아간다.
  //  새 배포로 제어자가 교체될 때만 새로고친다: src/main.tsx)
  await page.goto('/');
