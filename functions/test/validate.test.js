...
test('sanitizeFilename: 경로 조작 문자를 제거한다', () => {
  ...
  expect(sanitizeFilename('dub_fantasy_123_a1b2c3.mp4')).toBe('dub_fantasy_123_a1b2c3.mp4');
  expect(sanitizeFilename('한글파일명.mp4')).toBe('한글파일명.mp4');
});
...
test('validateUploadRequest: 정상 요청은 통과한다', () => {
  ...
  expect(result.safeName).toBe('dub_fantasy_1_abcdef.mp4');
  expect(result.buffer.equals(FAKE_MP4_BYTES)).toBe(true);
});
...
test('hasMp4Signature: ftyp 박스가 있으면 true, 없으면 false', () => {
  expect(hasMp4Signature(FAKE_MP4_BYTES)).toBe(true);
  expect(hasMp4Signature(Buffer.from('not an mp4 at all'))).toBe(false);
  expect(hasMp4Signature(Buffer.alloc(3))).toBe(false);
});

test('validateUploadRequest: filename/dataBase64 누락은 400', () => {
  expect(validateUploadRequest({}).ok).toBe(false);
  expect(validateUploadRequest({ filename: 'x.mp4' }).ok).toBe(false);
  expect(validateUploadRequest({ dataBase64: 'aGVsbG8=' }).ok).toBe(false);
});
...
test('validateUploadRequest: 파일명이 MAX_FILENAME_LEN을 넘으면 거부한다', () => {
  ...
  expect(validateUploadRequest(body).ok).toBe(false);
});
...
test('createRateLimiter: 창 안에서 한도를 넘으면 true를 반환한다', () => {
  ...
  expect(isRateLimited('1.2.3.4')).toBe(true);  // 4 - 한도 초과
});
