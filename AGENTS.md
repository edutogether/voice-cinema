...
### 8-1. 인앱 브라우저(카카오톡 등)에서만 다르게 보일 때
2026-09-09에 세 가지가 한꺼번에 드러났다.
- **`color-mix()`** — 옛 WebView가 모르면 **그 선언 전체를 무시**한다. 누를 때 강조가 통째로 사라졌다.
  색은 미리 계산해 넘긴다(`src/genres.ts`의 `withAlpha()`).
- **동시 재생 제한** — 여섯 장에 `play()`를 걸어도 하나만 살아남는다. 브라우저 이름이 아니라
  **`play()`가 거듭 거부되는 사실**로 판별해 한 장씩 돌려 재생한다.
- **웹폰트** — 이 앱은 폰트를 자체 호스팅한다(`fonts/` 원본 → 빌드가 서브셋). 그 전에는 설치된
  기기와 없는 기기가 다른 글꼴로 보였다.

### 8. "배포했는데 화면이 그대로"일 때
캐시가 세 겹이다: CDN 엣지 / 서비스워커 CacheStorage / 브라우저 일반 HTTP 캐시
**기기에서 확인이 안 될 때는 거의 항상 서비스워커 캐시다** — 재부팅·OS 업데이트로도 안 지워진다.
**비공개 탭으로 열면 즉시 최신본**이다. 새 버전이 오면 홈 화면에 알림이 뜨고 누를 때만 새로고침한다
...
`(승인 Bumm M/D)`를 덧붙인다. 브랜치 없이 `master`에 직접 커밋한다 — 배포가 `master` push에
걸려 있어, 라이브를 보며 바로 확인해야 하는 수정은 이 경로가 맞다.

다만 **규모가 큰 작업은 짧은 브랜치를 파고 PR을 열어 설명을 쓴 뒤 바로 머지**한다
(머지도 `master` push라 배포는 그대로 일어난다). 폰트 서브셋·서비스워커 구조 변경처럼
왜 그렇게 했는지가 남아야 하는 일이 그렇다. 급한 라이브 수정만 직접 커밋으로 둔다.
