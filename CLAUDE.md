...
- **백엔드 자동배포**: `.github/workflows/deploy.yml`(2026-08-29 추가) — 그 전까진 `firebase deploy`를 세션이 매번 손으로 실행해왔다. `CI` 워크플로우(lint+유닛테스트+E2E)가 성공한 뒤에만 `workflow_run`으로 이어져 `storage,functions`를 배포한다. **`FIREBASE_SERVICE_ACCOUNT` 저장소 시크릿이 아직 등록 안 돼 있어 지금은 job이 조용히 스킵된다** — 서비스 계정 키 발급은 GCP 콘솔 권한이 필요해 세션이 대신 못 함(poster-studio와 동일한 구조적 제약, `functions-deploy.yml` 패턴 그대로 가져옴). 대표가 콘솔에서 서비스 계정 만들고 키를 저장소 시크릿으로 등록하면 별도 코드 수정 없이 바로 동작한다. 등록 전까지는 여전히 수동 `firebase deploy` 필요.
