...
**정직하게 남기는 불확실성**: 이 세션의 도구(내장 샌드박스 브라우저, curl)로는 원래 증상 자체를 한 번도 재현하지 못했다 — 그래서 이 수정이 100% 확실한 근본 원인이라고 단정하지 않는다. 다만 (1) 증상(정적 뷰포트에서 이미지가 로드 자체가 안 됨)과 이 알려진 lazy-loading 실패 패턴이 정확히 일치하고, (2) 이 화면에서 지연로딩이 애초에 불필요했으므로, 되돌릴 이유 없는 안전한 수정이다. 재배포 후 대표님의 실제 화면 재확인이 최종 검증이며, 그래도 재현되면 원인을 처음부터 다시 조사해야 한다.
