...
4. **GCS 버킷 라이프사이클 규칙(`dubs/` prefix, 60일 삭제)** — 대표님이 콘솔에서 직접 만드셨다고 확인(2026-08-27). 세션은 이 환경에 gcloud/gsutil이 없어 CLI로 독립 검증은 못 했음(OAuth 토큰을 우회해서 직접 조회하려던 시도는 안전정책상 정당하게 차단됨, 더 시도하지 않았다) — 대표님 1차 진술을 신뢰하고 기록만 남긴다.
