...
- `docs/sw.js`(서비스워커)가 세 갈래로 서빙한다 — ①벤더 파일(ffmpeg 엔진 31MB, qrcode.js, App Check 번들)은 캐시우선 ②앱 셸(`./`, `index.html`, `app.js`, `logic.js`, `privacy.html`)은 네트워크우선·캐시폴백 ③클립은 한 번 불러온 장르부터 쌓이는 런타임 캐싱. Cloud Functions 호출은 건드리지 않는다. **`docs/` 아래 앱이 로드하는 파일을 추가·삭제·개명하면 이 목록(`PRECACHE_URLS`/`APP_SHELL_URLS`)을 같이 고치고 `CACHE_NAME` 버전을 반드시 올린다** — 빠뜨리면 오프라인에서 앱이 통째로 안 뜨거나, 이미 방문한 기기가 옛 화면을 계속 본다(둘 다 실제로 발생함, 자세한 경위는 `_docs/CHANGELOG.md`).
