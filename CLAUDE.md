...
- **백엔드 자동배포**: `.github/workflows/deploy.yml`(2026-08-29 추가, 2026-09-01 hosting 배포 단계 추가) — `CI` 워크플로우(lint+유닛테스트+E2E)가 성공한 뒤에만 `workflow_run`으로 이어져 `storage,functions`를 배포하고, 그 다음 `hosting:voice-cinema`를 배포한다. **`FIREBASE_SERVICE_ACCOUNT` 저장소 시크릿 등록 완료(2026-08-29, 대표 콘솔 작업)**이지만, **2026-09-01 기준 이 서비스계정에 `cleanupAfterCutoff`의 Cloud Scheduler 잡을 갱신할 권한(`cloudscheduler.jobs.update`)이 없어 functions 배포 단계에서 403으로 막히는 게 실제 코드 변경 배포로 처음 발견됨** — 그동안의 "성공"은 매번 함수 코드가 안 바뀌어 Skipped로만 지나갔기 때문(아래 "🔴 대표 조치 필요" 참고). 이 문제가 풀리기 전까지 hosting 자동배포 단계도 같이 막힌다(같은 job 안에서 이전 step 실패 시 이후 step 스킵) — 그래서 이번 클립교체+Hosting이전 배포는 이 세션이 로컬 `firebase deploy`로 직접 수행하고 실측 검증까지 완료했다.
...
- **D2(대표 결정) — Firebase App Check 도입, 프론트/서버 코드 대기 중**: App Check을 쓰려면 이 프로젝트에 등록된 Firebase Web App이 있어야 하는데 지금까지 하나도 없었다(`firebase apps:list WEB` → 0개, `docs/app.js`가 Firebase JS SDK 없이 순수 fetch만 씀). `firebase apps:create WEB "Voice Cinema Web"`로 Web App을 새로 등록했다(App ID `1:710797378638:web:5d8b0fe73666bb84026f1f`, `firebase apps:sdkconfig`로 언제든 config 재확인 가능). **여기서 막힘**: App Check의 reCAPTCHA v3 공급자 등록(사이트 키 발급)은 `firebase-tools`에 해당 CLI 명령 자체가 없어(`appcheck:*` 명령 없음 확인) **Firebase 콘솔에서만** 할 수 있다 — 대표 조치 필요 #6으로 아래에 추가. 사이트 키가 나오면, 이 세션(또는 후속 세션)이 (1) Firebase App/App Check JS SDK를 `docs/vendor/firebase/`에 로컬 벤더링(CSP `script-src 'self'` 유지 위해 CDN 안 씀) (2) `docs/app.js` 초기화 + `/upload` fetch에 `X-Firebase-AppCheck` 헤더 부착 (3) `functions/index.js`에 `firebase-admin`의 `appCheck().verifyToken()` 서버 검증 추가를 이어서 진행한다. 반쪽짜리 클라이언트 코드를 사이트 키 없이 미리 심어두면 오히려 실패하는 App Check 초기화 코드가 프로덕션에 배포되는 리스크라, 사이트 키 확보 전까지는 이 부분만 보류했다.

## 🔴 대표 조치 필요 — 아직 안 끝난 것 (2026-09-01 갱신)
...
2. **GCP 예산 알림/결제 한도 설정** — 콘솔 전용이라 세션이 못 함. 대표님 확인 결과 예산(budgets) 생성 화면 자체가 안 뜨는 상황 — 결제 계정 관리자 권한 문제로 추정, 원인 확인 중.
   - 콘솔 링크: `https://console.cloud.google.com/billing/budgets?project=inky-voice-cinema` (결제 계정 자체는 `https://console.cloud.google.com/billing/linkedaccount?project=inky-voice-cinema`에서 확인). 세션은 결제 계정 소유/권한 구조를 조회할 수단(gcloud 등)이 없어 원인을 더 좁히지 못했다 — 2026-09-01에 링크만 다시 확인해 추가.
...
5. **`FIREBASE_SERVICE_ACCOUNT`(CI 배포용 서비스계정)에 `roles/cloudscheduler.admin`(또는 최소 `cloudscheduler.jobs.update`) 역할 추가 필요(2026-09-01 발견)** — `cleanupAfterCutoff`(예약 함수)를 배포할 때 firebase-tools가 그 밑의 Cloud Scheduler 잡도 직접 갱신하는데, 지금 이 서비스계정엔 그 권한이 없어 403으로 막힌다. GCP 콘솔 IAM 권한 부여가 필요해 세션이 대신 못 함 — poster-studio가 겪은 `secretmanager.secrets.get`(secretAccessor만으론 부족, viewer도 필요했음)과 같은 종류의 세분화된 권한 문제. 이 항목이 풀리기 전까지 `deploy.yml`의 functions 배포 단계가 계속 막히고(hosting 배포 단계도 같은 job이라 연쇄로 스킵됨), 코드 변경이 있을 때마다 이 세션이 로컬 `firebase deploy`로 우회해야 한다.
   - 콘솔 링크: `https://console.cloud.google.com/iam-admin/iam?project=inky-voice-cinema` — GitHub Actions 배포용으로 이미 Firebase Admin류 역할이 부여된 서비스계정(2026-08-29에 대표가 직접 만든 것)을 찾아 거기에 `Cloud Scheduler 관리자`(`roles/cloudscheduler.admin`)를 추가로 부여하면 된다.
6. **Firebase App Check 콘솔 등록 필요(2026-09-01, 대표 승인 D2 후속)** — App Check로 `/upload`를 보호하려면 reCAPTCHA v3 공급자를 등록해 사이트 키를 발급받아야 하는데, 이 작업은 `firebase-tools`에 CLI 명령 자체가 없어 콘솔에서만 가능하다(Web App 등록은 세션이 CLI로 이미 완료 — 위 "종합감사 후속조치" 참고).
   - 콘솔 링크: `https://console.firebase.google.com/project/inky-voice-cinema/appcheck` → "앱 등록" → `Voice Cinema Web`(App ID `1:710797378638:web:5d8b0fe73666bb84026f1f`) 선택 → 공급자 **reCAPTCHA v3** 선택(Firebase가 자동으로 사이트 키를 만들어줌, 별도 Google reCAPTCHA 계정 불필요). 등록되면 이 세션이 이어서 프론트/서버 코드를 완성한다.
