...
🟠 **점수에는 "언제 트리의 점수인지"가 붙는다** — **`10/10` (2026-09-10 트리 기준)**이다.
그 뒤로 커밋이 쌓였으므로 **지금 트리의 점수가 아니다.**

**커밋 수를 여기 숫자로 박지 않는다** — 다음 커밋에 바로 낡는다. 필요할 때 센다:

```bash
git rev-list --count voice-cinema-freeze-20260910-audited-100..master
```
...
- **Storage `dubs/`는 학생 완성본이 들어가는 운영 경로다.** 2026-09-11 확인 시점에는 7개가 들어
  있었고 **전부 검증 찌꺼기(5~12바이트 더미 포함), 학생 것은 0개**였다 — 행사 전이라 당연하다.
  자동삭제는 2026-12-01부터라 그때까지 저절로 지워지지 않지만, **급히 지울 것은 아니다.**
  🟠 **"지워야 한다"고 올리기 전에 `gcloud storage ls gs://inky-voice-cinema.firebasestorage.app/dubs/`로
  안을 먼저 볼 것** — 2026-09-11에 안 보고 올렸다가 "삭제를 왜 해, 더빙하면 거기로 가는 거 아니야?"라는
  되물음을 받았다. **운영 경로를 비우라고 올리는 것은 위험한 요청이다.**
